...
function Page() {
  ...
  function deleteHotel(id: string) {
    ...
    fetch(`http://localhost:5000/hotels/${id}`, options)
      .then((response) => response.json())
      .then((res) => {
        setModalState("finished");
        // remove room from fetched rooms to decrease amount of requests (refetch)
  ...
  function handleModal(hotel: Hotel) {
    ...
    <>
      <div
        className={`fixed w-full h-screen z-50 items-center justify-center bg-sea-80 bg-opacity-50 p-4 ${
          isModalVisible && modalHotel ? "flex" : "hidden"
        }`}
      >
        <div className={`flex flex-col gap-4 bg-white p-8 rounded-lg `}>
          {modalState == "confirmation" && modalHotel && (
            <>
              <Heading size={4}>Delete hotel</Heading>
              <p>{`Are you sure you want to delete ${modalHotel?.name} hotel?`}</p>
              <div
                ...
                {hotels?.map((hotel) => {
                  return (
                    ...
                    >
                      ...
                      >
                        <Heading size={6}>{`${hotel.name} hotel`}</Heading>
                        <p>{`${hotel.region}`}</p>
                      </div>
                      <div
