...
function Page() {
  ...
  const [modalRoom, setModalRoom] = useState<HotelRoom>();
  ...
  function deleteRoom(id: string) {
    ...
    fetch(`http://localhost:5000/hotel-rooms/${id}`, options)
      .then((response) => response.json())
      .then((res) => {
  ...
  function handleModal(room: HotelRoom) {
      ...
      >
        <div className={`flex flex-col gap-4 bg-white p-8 rounded-lg `}>
          {modalState == "confirmation" && modalRoom && (
            <>
              <Heading size={4}>Delete room</Heading>
              <p>{`Are you sure you want to delete ${modalRoom?.name} room?`}</p>
              <div
